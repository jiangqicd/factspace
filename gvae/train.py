...
def train(trainingdata_path, rule_path, model_save_path):
    # 0. load dataset and rules
    h5f = h5py.File(trainingdata_path, 'r')
    data = h5f['data'][:]
    h5f.close()
    ...
    model_save = model_save_path + 'vae_H%d_D%d_C%d%d%d_%d%d%d_L%d_B%d.hdf5' % (
        args.hidden, args.dense, args.conv1[0], args.conv2[0], args.conv3[0], args.conv1[1], args.conv2[1],
        args.conv3[1],
        args.latent, args.batch)
    model = ModelVAE()
