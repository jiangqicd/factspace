...
def main():
    ...
    h5f = h5py.File('./trainingdata/happiness/train.h5', 'r')
    data = h5f['data'][:]
    h5f.close()

    rules = []
    with open('./trainingdata/happiness/rules-cfg.txt', 'r') as inputs:
        for line in inputs:
            line = line.strip()
            rules.append(line)
    ...
    model_save = './trained/happiness/vae_H%d_D%d_C%d%d%d_%d%d%d_L%d_B%d.hdf5' % (args.hidden, args.dense, args.conv1[0], args.conv2[0], args.conv3[0], args.conv1[1], args.conv2[1], args.conv3[1], args.latent, args.batch)
    model = ModelVAE()
